...
function getRectangleString(width, height) {
  ...
  const midleLines = `│${' '.repeat(width - 2)}│\n`;
  return `${firstLine}${midleLines.repeat(height - 2)}${lastLine}`;
}
...
function encodeToRot13(str) {
  ...
  const output = '?! NOPQRSTUVWXYZABCDEFGHIJKLM?! nopqrstuvwxyzabcdefghijklm';
  const resultArr = [];
  for (let i = 0; i < str.length; i++) {
    const index = input.indexOf(str[i]);
    resultArr.push(output[index]);
  }
